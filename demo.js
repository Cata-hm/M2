...
createdBy.innerHTML = createdBy.innerHTML + " Catalina Hernández Mejía";
...
// { description: "Tarea a realizar", complete: false} ---> esta es la apariencía de nuestro objeto
...
function buildToDo(todo, index) {
  // Tu código acá:
  // toDo --> {description: "Tarea a realizar", complete: False}

  var toDoShell = document.createElement("div"); // punto 1)
  toDoShell.className = "toDoShell"; // punto 2) con este lo que hago es que piso lo q haya en la clase con toDoShell
  //toDoShell.classList.add = "toDoShell"; --> la diferencia es que esta añade otra clase
  
  var toDoText = document.createElement("span"); // punto 3)
  toDoText.innerHTML = todo.description; // punto 4) acá lo q hago es agregarle al span la descripción asi --> <span>"Tarea a realizar"</span>
  toDoText.id = index; // punto 5) también s epuede hacer así --> setAttribute("id",index)

  if(todo.complete) { // punto 6) si es true
    toDoText.className = "completeText"; // se asigna a 'toDoText' la clase 'completeText'
  }

  toDoShell.appendChild(toDoText); // punto 7) quedando si --> <div> <span> "Tarea a realizar" </span> </div>

  toDoText.addEventListener("click", completeToDo); // viene de completeToDo
...
function buildToDos(toDos) {
  // Tu código acá:
  var array = toDos.map(buildToDos) //acá es como si estuvieramos usando la función de builToDos, línea 53, la ejecutará por cada elemento
  return array; // cada elemento ejecutado hace parte del array, al final devuelve el array con todos los elementos
}
...
function displayToDos() {
  // Tu código acá:
  var toDoContainer = document.getElementById("toDoContainer"); // paso 1) <div></div> es un div vacío
  toDoContainer.innerHTML = ""; // paso 2)

  var aux = toDoContainer(toDoItems); // paso 3)
  aux.forEach(function(element) { // paso 4) con el for se hace la iteración
    toDoContainer.appendChils(element);
  });
}
...
function addToDo() {
  // Tu código acá:
   var toDoInput = document.getElementById("toDoInput"); // paso 1)
   var newToDo = new ToDo(ToDoInput.value); // paso 2)

   toDoItems.push(newToDo); // paso 3)
   toDoInput.value = ""; // esto lo q hace es borrar lo que se escribió en la entrada despues de ingresarlo, para ingresar uno nuevo
...
displayToDos(); //paso 5) invocamos la función
